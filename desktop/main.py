...
from core.network import collect_device_info, send_command
from core.ota import DEFAULT_OTA_URLS, perform_ota_upgrade
from core.parser import DeviceResult, normalize_command_library
from core.utils import build_ip_list, format_log_line
...
OTA_URLS = dict(DEFAULT_OTA_URLS)
...
def load_command_library_from_json(parent=None):
    ...
    candidates = [
        base_dir / "tasmota_commands.json",
        base_dir.parent / "tasmota_commands.json",
    ]
    path = None
    ...
    records = normalize_command_library(data)
...
class Worker(QObject):
    ...
    def _log(self, ip, name, msg, tag="INFO"):
        self.log_line.emit(format_log_line(ip, name, msg, tag=tag), tag)

    async def _send_backlog(self, client: httpx.AsyncClient, ip: str, backlog: str):
        await send_command(
            client,
            ip,
            f"Backlog {backlog}",
    ...
    async def _run_upgrade(self, client: httpx.AsyncClient, ip: str, hardware: str, name: str) -> bool:
        def _ota_log(message: str, tag: str = "INFO"):
            self._log(ip, name, message, tag=tag)

        return await perform_ota_upgrade(
            client,
            ip,
            hardware=hardware,
            log=_ota_log,
                ...
                if info.Ok:
                    ...
                    if ip in self.selected_ips:
                        if self.do_upgrade:
                            ...
                            ok = await self._run_upgrade(client, ip, info.Hardware, info.Name)

                            # If upgrade worked and backlog is enabled → send backlog
                            if ok and self.send_backlog and self.commands:
                                backlog = "; ".join(self.commands)
                                self._log(ip, info.Name, "Sending backlog after upgrade...", tag="CMD")
                                await self._send_backlog(client, ip, backlog)

                        elif self.send_backlog and self.commands:
                            # --- Backlog Only Path ---
                            backlog = "; ".join(self.commands)
