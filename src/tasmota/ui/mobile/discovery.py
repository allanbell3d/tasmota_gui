...
from .widgets.common import BorderedGridLayout
...
class DiscoveryPanel(BoxLayout):
    ...
    def update_progress(self, done: int, total: int):
        def _apply(_: float):
            ...
            if total:
                pct = min(pct, 100)
            capped_done = min(done, total) if total else 0
            self.progress_bar.max = max(total, 1)
            self.progress_bar.value = capped_done
            self.progress_label.text = f"Scan Progress: {pct}% ({done} / {total})"
