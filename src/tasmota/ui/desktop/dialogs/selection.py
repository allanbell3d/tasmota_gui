...
from tasmota.constants import OTA_URLS
...
class SelectionWindow(QDialog):
    ...
    def apply_filters(self):
        ...
        for i in range(self.table.rowCount()):
            ...
            if selected_only:
                cmd_widget = self.table.cellWidget(i, 0)
                fw_widget = self.table.cellWidget(i, 1)
                if cmd_widget is None or fw_widget is None:
                    continue  # Skip rows with missing widgets
                c = cmd_widget.isChecked()
                f = fw_widget.isChecked()
