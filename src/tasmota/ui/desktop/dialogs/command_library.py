...
from tasmota.core.commands import CommandRecord, extract_categories
...
class CommandLibraryDialog(QDialog):
    ...
    def _populate_category_filter(self):
        ...
        # Extract categories using shared helper (filters to CommandRecord only)
        records = [r for r in self.commands if isinstance(r, CommandRecord)]
        categories, has_uncategorized = extract_categories(records)
